...
//
// The native addon is the part that usually cannot cross-build: webview links against Cocoa,
// WebKitGTK or WebView2-COM, so it needs the target platform's own SDK. macOS is the exception
// — one Xcode targets both arm64 and x86_64 — which is how the Intel mac is built on an Apple
// Silicon runner. Any other cross build needs out/webview_napi-<platform>-<arch>.node to
// already be there; see scripts/addon.mjs.
