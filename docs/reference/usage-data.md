...
| Objects, and the width of the cap | the same rows, counted                                                                                     | folders are not objects and are not counted                                                           |
...
| Which sector                      | `object.category`, worked out from the file extension when the row is written                              | the path is never read: a `logs/` folder of screenshots is screenshots, and gzipped logs are archives |
...
| A grey ball                       | the same ages, at 365 days                                                                                 | a ball's distance from its pole is its age too                                                        |
...
| How often a ball was opened       | that object's own rows in `objectView`, plus the gateway's calls naming it                                 | on the ball's card, not drawn on the ball                                                             |
| Pale top of the stack             | the size of the cached copies on this disk, per root and category; `object.onDiskSize` for a synced folder | a cloud root's is measured by walking the cache folder, which is its only index                       |
| The ring on the cap               | how many reads the pole has had, drawn as an area on the same scale as the cap                             | a ring the size of its cap is about one read per object                                               |
| Pale arc of the base              | `objectView` — one row per object you opened in this window                                                | written by the window's own heartbeat and by nothing else                                             |
| Dark arc of the base              | the agent gateway's log in `~/.objectexplorer/mcp`                                                         | one line per call, matched back to an object by the URI the agent used                                |
| Thickness of the base             | bytes those reads moved, from `objectAccess`, against the busiest pole on the disc                         | one figure for the pole, not split between person and agent — see below                               |
| An arch between two poles         | the same bytes found in both                                                                               | matched by `apiResponse.etag` where both sides have one, otherwise by name and size                   |
| Saved, on the card                | three terms, below                                                                                         |                                                                                                       |
...
the bucket's region. Where a class bills a minimum per object — S3's Standard-IA, One Zone-IA and
Glacier Instant Retrieval bill every object as at least 128 KiB, Glacier Flexible Retrieval and
Deep Archive as at least 40 KiB, and Cloud Storage's Nearline, Coldline and Archive as at least
128 KiB — the pole is priced at whichever is larger:
...
bucket you added and never opened has no pole — it has a bare post, which is the honest answer. Open
...
answered with, so the two *arcs* of the base are told apart by its own log. The bytes a read moved are
recorded wherever they were served, by the window and the gateway alike, so the base's *thickness*
is one figure for the pole and is never split between the two.
...
**Six objects, not the tail.** A pole draws six objects as balls, every one a real object that can
be clicked and named. The rest of what it holds is not drawn one by one; its ages are the bars on the
card.
...
| provider folders | the cached copies, whose size on disk is the pale top of a stack             |
| `mcp/`           | one file per agent session, which is the dark arc of a base                  |
